...
class DynamicTable {
        ...
        data.forEach((row, index) => {
            ...
            fields.forEach(field => {
                ...
                if (field.isAction) {
                    ...
                    console.log('DynamicTable - current_stage:', row.current_stage, 'accept_offer:', row.accept_offer, 'no_report_reason:', row.no_report_reason);
                    ...
                    console.log('DynamicTable - stageText:', stageText);
    ...
    renderActions(row) {
        ...
        switch (row.current_stage) {
            case 'application':
                // 投递简历阶段 - 显示初试处理按钮
                actions.push(`<button class="btn-process" data-id="${row.id}" data-stage="first">处理</button>`);
                break;
            case 'first_interview':
                // 关键修复：初试通过但未进行复试，显示复试处理按钮
            ...
            case 'second_interview':
                // 关键修复：复试通过但未设置accept_offer，显示填写录用信息按钮
                ...
                if (row.current_status === 'pending') {
                    actions.push(`<button class="btn-process" data-id="${row.id}" data-stage="second">处理</button>`);
